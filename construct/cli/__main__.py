...
def on_action_before(ctx):
    ...
    if ctx.kwargs:
        args_section = format_section(
            ARGUMENTS_TITLE,
            data=[(k, v) for k, v in ctx.kwargs.items()],
            lcolor=style.bright
        )
        print(args_section + '\n')
    ...
    ctx_section = format_section(
        ACTION_CONTEXT_TITLE,
        data=[(k, ctx[k]) for k in ctx.keys if ctx[k]],
        lcolor=style.bright
    )
    print(ctx_section + '\n')
...
def on_request_status_change(request, last_status, status):
    msg = '    {}'.ljust(40, '.').format(request.task.identifier) + status
    print(msg)
...
def main():
    ...
    for identifier, action in construct.actions.collect().items():
